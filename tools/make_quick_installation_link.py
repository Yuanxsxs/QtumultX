...
def Output(raw,tag = "Yuan's Selfuse Rewrite"):
    ...
    set = re.search("^https?:\/\/raw\.githubusercontent\.com\/(?P<author>.*?)\/.*\/(?P<tag>.*)\.[(conf)|(snippet)|(txt)(json)]",raw)
...
def main():
    raw = pyperclip.paste()
    research = re.search("^https?:\/\/raw\.githubusercontent\.com\/(?P<author>.*?)\/.*\/(?P<tag>.*)\.[(conf)|(snippet)|(txt)(json)]",raw)
    if research == None:
